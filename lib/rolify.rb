...
module Rolify
  ...
  def rolify(options = {})
    ...
    if Rolify.orm == "active_record"
      ...
      join_table_name = "#{self.to_s.tableize}_#{options[:role_cname].tableize}"
      join_table_name.gsub!('/', '_') if options[:role_cname].include?('::')

      rolify_options.merge!({ :join_table => join_table_name })
